...
func TestGroupMatcherAdvanced(t *testing.T) {
	...
	t.Run("multiple matchers first match wins", func(t *testing.T) {
		executed := false
		matcher := NewGroupMatcher(
			NewRegexpMatcher("first.*", func(match Result, message msg.Message) {
				executed = true
			}),
			NewRegexpMatcher("second pattern", func(match Result, message msg.Message) {
...
func TestMatcherIntegration(t *testing.T) {
	t.Run("prefix with admin matcher", func(t *testing.T) {
		...
		adminMatcher := NewAdminMatcher(
			cfg.AdminUsers,
			slackClient,
			NewPrefixMatcher("admin command", func(match Result, message msg.Message) {
				executed = true
