...
# Installation
1. clone the project
2. create config file called `config.yaml` (you can take a look at `example_config/config.yaml`)
...
**Attention**: This command will use the config from /example_config/ only. 
-> add slack.token in example_config/config.yaml and run:
```
